...
!    This is the HYBRID version using HYBRID FORMULATION=TOTAL 
...
! ---------------------------------------------------------------------
! 
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3,3) :: BeOLD
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3,3) :: BeOLDINV
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3,3) :: CiOLD
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3,3) :: CiOLDINV
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3,3) :: BeTR
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3,3) :: BeTR_
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3) :: PVBeTR
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3,3) :: PDBeTR
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3) :: EPSeTR
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3) :: EPSe
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3) :: DELEPSe
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3,3) :: Be
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3) :: PVBe
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3) :: PVBeBAR     
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3) :: RESVEC
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3,3) :: KMAT
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3,3) :: KINV
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3,3) :: TAUNEQ
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3,3) :: TAUNEQD
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3,3) :: TAUNEQH 
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3) :: PVTAUNEQ
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3) :: PVTAUNEQD
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7) :: PVTAUNEQH
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3,3) :: PDTAUNEQ
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3,3):: SIGMANEQ
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3,3) :: DPVTAUDEPSe
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3,3) :: DPVDEVTAUDEPSe
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7) :: DPVHYDTAUDEPSe
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3,3) :: CALG
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3,3,3,3) :: L4NEQ
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3,3,3,3) :: C4NEQ
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3,3,3,3) :: C4NEQJ
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,6,6) :: CNEQ
! 
! ---------------------------------------------------------------------
...
!     DI2DEPS        - D(I2) / D(EPS)
!     DUEDJHAT       - DU/DJHAT = ...
!     D2UEDJHAT2     - D2U/DJHAT2 = ...
!     D3UEDJHAT3     - D3U/DJHAT3 = ...
!     PHAT           - HYDROSTATIC PRESSURE IN HYBRID FORMULATION
!     AKHAT2         - J * D2UEDJHAT2
!     AKHAT3         - J * D3UEDJHAT3
!  
! ---------------------------------------------------------------------  
!    
      ...
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7) ::  ALPHANEQ
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7) ::  DALPHANEQDI1
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7) ::  DALPHANEQDI2
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7) ::  BETANEQ
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7) ::  DBETANEQDI1
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7) ::  DBETANEQDI2
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3) :: DI1eDEPSe
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7,3) :: DI2eDEPSe

      DOUBLE PRECISION DUEDJHAT
      DOUBLE PRECISION D2UEDJHAT2
      DOUBLE PRECISION D3UEDJHAT3
      DOUBLE PRECISION DUEDJHATNB
      DOUBLE PRECISION D2UEDJHAT2NB
      DOUBLE PRECISION D3UEDJHAT3NB
      DOUBLE PRECISION PHAT
      DOUBLE PRECISION AKHAT2
      DOUBLE PRECISION AKHAT3
!
! ---------------------------------------------------------------------
!     LOCAL VARIABLES
! ---------------------------------------------------------------------
...
!     AJHAT      - ALTERNATE VARIABLE IN PLACE OF J IN HYBRID FORMULATION
!  
! ---------------------------------------------------------------------
! 
      DOUBLE PRECISION MU
      DOUBLE PRECISION KELAS
      DOUBLE PRECISION Jv_n
      DOUBLE PRECISION Jv_new

      DOUBLE PRECISION DET
      DOUBLE PRECISION DETOLD
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7) :: MUVIS
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7) :: KVIS
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7) :: ETADEV
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7) :: ETAVOL
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7) :: Je
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7) :: Jv
      DOUBLE PRECISION, DIMENSION((NSTATV - 4)/7) :: NORMRES

      DOUBLE PRECISION AJHAT
...
!     CALCULATE JACOBIAN OF DEFORMATION GRADIENT
     ...
     3         -DFGRD1(2,3)*DFGRD1(3,2)*DFGRD1(1,1)
      END IF
! 
!     ALTERNATE J IN HYBRID FORMULATION
...
!     ASSIGN ELASTIC PROPERTIES TO CONSTANTS
      ...
      KELAS       = PROPS(12)
! 
!     1ST AND 2ND DERIVATIVES OF HYDROSTATIC EQUILIBRIUM PART IN HYBRID FORMULATION
! Set to zero and for the loop over N_B
      DUEDJHATNB = ZERO 
      D2UEDJHAT2NB = ZERO 
      D3UEDJHAT3NB = ZERO 
...
! _________________________________________________________
      DUEDJHAT    = (KELAS/TWO)*(AJHAT - ONE/AJHAT) 
      D2UEDJHAT2  = (KELAS/TWO)*(ONE + ONE/(AJHAT*AJHAT)) 
      D3UEDJHAT3  = -KELAS/(AJHAT*AJHAT*AJHAT) 
      DO N_B = 1, N_BRANCHES
!       DEFINING VISCOELASTIC INTERNAL STATE VARIABLES FOR FIRST ITERATION
        IF (KSTEP.EQ.1 .AND. KINC.EQ.1) THEN
              STATEV(1 + 7*(N_B - 1)) = ONE
              STATEV(2 + 7*(N_B - 1)) = ONE
              STATEV(3 + 7*(N_B - 1)) = ONE
              STATEV(4 + 7*(N_B - 1)) = ZERO
              STATEV(5 + 7*(N_B - 1)) = ZERO
              STATEV(6 + 7*(N_B - 1)) = ZERO
              STATEV(7 + 7*(N_B - 1)) = ONE
        END IF
!     
!       GET VISCOELASTIC INTERNAL STATE VARIABLES FROM PREVIOUS ITERATION (N-1)
        BeOLD(N_B,1,1) = STATEV(1 + 7*(N_B - 1))
        BeOLD(N_B,1,2) = STATEV(4 + 7*(N_B - 1))
        BeOLD(N_B,1,3) = STATEV(5 + 7*(N_B - 1))
        BeOLD(N_B,2,1) = STATEV(4 + 7*(N_B - 1))
        BeOLD(N_B,2,2) = STATEV(2 + 7*(N_B - 1))
        BeOLD(N_B,2,3) = STATEV(6 + 7*(N_B - 1))
        BeOLD(N_B,3,1) = STATEV(5 + 7*(N_B - 1))
        BeOLD(N_B,3,2) = STATEV(6 + 7*(N_B - 1))
        BeOLD(N_B,3,3) = STATEV(3 + 7*(N_B - 1))
        Jv(N_B) = STATEV(7 + 7*(N_B - 1))
! 
...
!     VISCOELASTIC LOOP
      DO N_B = 1, N_BRANCHES
        ...
        CiOLD(N_B,:,:)=MATMUL(TRANSPOSE(DFGRD0),
     1          MATMUL(BeOLDINV(N_B,:,:),DFGRD0))
        CALL M33INV(CiOLD(N_B, :, :), CiOLDINV(N_B, :, :), OK_FLAG)
        BeTR(N_B,:,:)=MATMUL(DFGRD1,
     1          MATMUL(CiOLDINV(N_B,:,:),TRANSPOSE(DFGRD1)))
      END DO
! 
! ---------------------------------------------------------------------
...
!---------------------------------------
!   Never change as the hydrostatic contribution is calculated later
!   due to the HYBRID formulation
!---------------------------------------
          PVTAUNEQH(N_B) = (ZERO)*(Je(N_B)**2-ONE)
...
!---------------------------------------
!   Never change as the hydrostatic part is calculated later due to 
!   HYBRID formulation
!---------------------------------------
          DPVHYDTAUDEPSe(N_B) =ZERO*Je(N_B)*Je(N_B)
...
!       2. PVTAUNEQ
        DO I=1,3
              PVTAUNEQ(N_B,I) = PVTAUNEQD(N_B,I) 
        END DO
! 
!       3. PDTAUNEQ
        DO I=1,3
        ...
        END DO
! 
!       4. CALCULATING DEVIATORIC, TOTAL NEQ KIRCHHOFF STRESS TENSOR
        DO I=1,3
          DO J=1,3
            TAUNEQD(N_B,J,I)=PVTAUNEQD(N_B,1)*PDTAUNEQ(N_B,1,J)
     1              *PDTAUNEQ(N_B,1,I)
     2              + PVTAUNEQD(N_B,2)*PDTAUNEQ(N_B,2,J)*PDTAUNEQ(N_B,2,I)
     3              + PVTAUNEQD(N_B,3)*PDTAUNEQ(N_B,3,J)*PDTAUNEQ(N_B,3,I)
          END DO
        END DO
! 
!       5. CALG
!       - Calculating Calculating d(PVTAUNEQ)/dEPSe
        DO I=1,3
          DO J=1,3
            DPVTAUDEPSe(N_B,J,I)=DPVDEVTAUDEPSe(N_B,J,I) 
          END DO
        END DO
!       - Calculating CALG
         DO I = 1,3
            DO J = 1,3
              KMAT(N_B,I,J) = IDT2(I,J)+DTIME*
     1           (ONE/(TWO*ETADEV(N_B))*DPVDEVTAUDEPSe(N_B,I,J))
            END DO
          END DO
!         - Calculating KINV
          CALL M33INV(KMAT(N_B, :, :), KINV(N_B, :, :), OK_FLAG)
...
!     SET INTERNAL STATE-VARIABLE FROM FINAL LOCAL ITERATION
      DO N_B = 1, N_BRANCHES
        STATEV(1 + 7*(N_B - 1)) = Be(N_B,1,1)
        STATEV(2 + 7*(N_B - 1)) = Be(N_B,2,2)
        STATEV(3 + 7*(N_B - 1)) = Be(N_B,3,3)
        STATEV(4 + 7*(N_B - 1)) = Be(N_B,1,2)
        STATEV(5 + 7*(N_B - 1)) = Be(N_B,1,3)
        STATEV(6 + 7*(N_B - 1)) = Be(N_B,2,3)
      END DO
!
! ---------------------------------------------------------------------
...
!       1. CALCULATE COEFFICIENTS FOR AABB
        DO J=1,3
          DO K=1,3
            L4NEQ(N_B,J,J,K,K)=(CALG(N_B,J,K)-PVTAUNEQD(N_B,J) 
     1           *TWO*IDT2(J,K))/(PVBeTR(N_B,J)*PVBeTR(N_B,K))
          END DO
        END DO 
...
!           - In case of Equal PVBeTR, Use Alternate Form
            IF (ABS(PVBeTR(N_B,J)-PVBeTR(N_B,K)).LT.EPS) THEN
              L4NEQ(N_B,J,K,J,K) = (L4NEQ(N_B,J,J,J,J)
     1                           -L4NEQ(N_B,J,J,K,K))/TWO
            ELSE
!
              L4NEQ(N_B,J,K,J,K)=((PVTAUNEQD(N_B,K)/PVBeTR(N_B,K))-
     1                          (PVTAUNEQD(N_B,J)/PVBeTR(N_B,J)))
...
!       CALCULATE JAUMANN RATE FORM OF MATERIAL TENSOR
     ...
     1              + (1 - DAMAGED_OLD) * (IDT2(I,K)*TAUNEQD(N_B,J,L)
     1              +IDT2(I,L)*TAUNEQD(N_B,J,K)
     1              +IDT2(J,K)*TAUNEQD(N_B,I,L)
     1              +IDT2(J,L)*TAUNEQD(N_B,I,K))/(TWO*DET)        
              END DO
            END DO
          END DO
        END DO
...
!     CALCULATING PRINCIPAL VALUES OF DEVIATORIC KIRCHOFF STRESS
     ...
     1            + BETAEQ * ((FOUR/THREE)*(PVBBAR(1)**TWO)
     1              - (TWO/THREE)*(PVBBAR(2)**TWO) 
     1              - (TWO/THREE)*(PVBBAR(3)**TWO))
      PVTAUEQD(2) = ALPHAEQ * ((FOUR/THREE)*PVBBAR(2)
     1              - (TWO/THREE)*PVBBAR(3) 
     1              - (TWO/THREE)*PVBBAR(1))
     1            + BETAEQ * ((FOUR/THREE)*(PVBBAR(2)**TWO)
     1              - (TWO/THREE)*(PVBBAR(3)**TWO) 
     1              - (TWO/THREE)*(PVBBAR(1)**TWO))
      PVTAUEQD(3) = ALPHAEQ * ((FOUR/THREE)*PVBBAR(3)
     1              - (TWO/THREE)*PVBBAR(1) 
     1              - (TWO/THREE)*PVBBAR(2))
     1            + BETAEQ * ((FOUR/THREE)*(PVBBAR(3)**TWO)
     1              - (TWO/THREE)*(PVBBAR(1)**TWO) 
     1              - (TWO/THREE)*(PVBBAR(2)**TWO))
! 
!     CALCULATING PRINCIPAL VALUES OF TOTAL KIRCHOFF STRESS
      PVTAUEQ(1) = PVTAUEQD(1) 
      PVTAUEQ(2) = PVTAUEQD(2)
      PVTAUEQ(3) = PVTAUEQD(3) 
! 
!     CALCULATING DEVIATORIC, TOTAL EQ CAUCHY STRESS TENSOR
      DO J=1,3
        DO K=1,3
          SIGMAEQD(K,J) = (PVTAUEQD(1)*PDBTOT(1,K)*PDBTOT(1,J)
     1                    +PVTAUEQD(2)*PDBTOT(2,K)*PDBTOT(2,J)
     1                    +PVTAUEQD(3)*PDBTOT(3,K)*PDBTOT(3,J))/DET
          SIGMAEQ(K,J) = SIGMAEQD(K,J) 
        END DO
      END DO
! 
...
!     CALCULATING TOTAL CAB
     ...
     1         + ALPHAEQ *
     1           (((FOUR*FOUR)/NINE)*PVBBAR(1)
     1            + (FOUR/NINE)*(PVBBAR(2) + PVBBAR(3)))
     1         + (DBETAEQDI1*DI1DEPS(1) + DBETAEQDI2*DI2DEPS(1)) *
     1         ((FOUR/THREE)*(PVBBAR(1)**TWO)
     1          - (TWO/THREE)*((PVBBAR(2)**TWO) + (PVBBAR(3)**TWO)))
     1         + BETAEQ *
     1           (((FOUR*EIGHT)/NINE)*(PVBBAR(1)**TWO)
     1            + (EIGHT/NINE)*((PVBBAR(2)**TWO) + (PVBBAR(3)**TWO)))
      CAB(2,2) = (DALPHAEQDI1*DI1DEPS(2) + DALPHAEQDI2*DI2DEPS(2)) *
     1         ((FOUR/THREE)*PVBBAR(2)
     1          - (TWO/THREE)*(PVBBAR(3) + PVBBAR(1)))
     1         + ALPHAEQ * 
     1           (((FOUR*FOUR)/NINE)*PVBBAR(2)
     1            + (FOUR/NINE)*(PVBBAR(3) + PVBBAR(1)))
     1         + (DBETAEQDI1*DI1DEPS(2) + DBETAEQDI2*DI2DEPS(2)) *
     1         ((FOUR/THREE)*(PVBBAR(2)**TWO)
     1          - (TWO/THREE)*((PVBBAR(3)**TWO) + (PVBBAR(1)**TWO)))
     1         + BETAEQ * 
     1           (((FOUR*EIGHT)/NINE)*(PVBBAR(2)**TWO)
     1            + (EIGHT/NINE)*((PVBBAR(3)**TWO) + (PVBBAR(1)**TWO)))
      CAB(3,3) = (DALPHAEQDI1*DI1DEPS(3) + DALPHAEQDI2*DI2DEPS(3)) *
     1         ((FOUR/THREE)*PVBBAR(3)
     1          - (TWO/THREE)*(PVBBAR(1) + PVBBAR(2)))
     1         + ALPHAEQ * 
     1           (((FOUR*FOUR)/NINE)*PVBBAR(3)
     1            + (FOUR/NINE)*(PVBBAR(1) + PVBBAR(2)))
     1         + (DBETAEQDI1*DI1DEPS(3) + DBETAEQDI2*DI2DEPS(3)) *
     1         ((FOUR/THREE)*(PVBBAR(3)**TWO)
     1          - (TWO/THREE)*((PVBBAR(1)**TWO) + (PVBBAR(2)**TWO)))
     1         + BETAEQ *
     1           (((FOUR*EIGHT)/NINE)*(PVBBAR(3)**TWO)
     1            + (EIGHT/NINE)*((PVBBAR(1)**TWO) + (PVBBAR(2)**TWO)))
      CAB(1,2) = (DALPHAEQDI1*DI1DEPS(2) + DALPHAEQDI2*DI2DEPS(2)) *
     1         ((FOUR/THREE)*PVBBAR(1)
     1          - (TWO/THREE)*(PVBBAR(2) + PVBBAR(3)))
     1         + ALPHAEQ *
     1           ((FOUR/NINE)*PVBBAR(3)
     1            - (EIGHT/NINE)*(PVBBAR(1) + PVBBAR(2)) )
     1         + (DBETAEQDI1*DI1DEPS(2) + DBETAEQDI2*DI2DEPS(2)) *
     1         ((FOUR/THREE)*(PVBBAR(1)**TWO)
     1          - (TWO/THREE)*((PVBBAR(2)**TWO) + (PVBBAR(3)**TWO)))
     1         + BETAEQ *
     1           ((EIGHT/NINE)*(PVBBAR(3)**TWO)
     1           -((TWO*EIGHT)/NINE)*((PVBBAR(1)**TWO)+(PVBBAR(2)**TWO)))
      CAB(1,3) = (DALPHAEQDI1*DI1DEPS(3) + DALPHAEQDI2*DI2DEPS(3)) *
     1         ((FOUR/THREE)*PVBBAR(1)
     1          - (TWO/THREE)*(PVBBAR(2) + PVBBAR(3)) )
     1         + ALPHAEQ * 
     1           ((FOUR/NINE)*PVBBAR(2)
     1            - (EIGHT/NINE)*(PVBBAR(3) + PVBBAR(1)) )
     1         + (DBETAEQDI1*DI1DEPS(3) + DBETAEQDI2*DI2DEPS(3)) *
     1         ((FOUR/THREE)*(PVBBAR(1)**TWO)
     1          - (TWO/THREE)*((PVBBAR(2)**TWO) + (PVBBAR(3)**TWO)) )
     1         + BETAEQ *
     1           ((EIGHT/NINE)*(PVBBAR(2)**TWO)
     1         -((TWO*EIGHT)/NINE)*((PVBBAR(3)**TWO)+(PVBBAR(1)**TWO)))
      CAB(2,3) = (DALPHAEQDI1*DI1DEPS(3) + DALPHAEQDI2*DI2DEPS(3)) *
     1         ((FOUR/THREE)*PVBBAR(2)
     1          - (TWO/THREE)*(PVBBAR(3) + PVBBAR(1)) )
     1         + ALPHAEQ * 
     1           ((FOUR/NINE)*PVBBAR(1)
     1            - (EIGHT/NINE)*(PVBBAR(2) + PVBBAR(3)) )
     1         + (DBETAEQDI1*DI1DEPS(3) + DBETAEQDI2*DI2DEPS(3)) *
     1         ((FOUR/THREE)*(PVBBAR(2)**TWO)
     1          - (TWO/THREE)*((PVBBAR(3)**TWO) + (PVBBAR(1)**TWO)) )
     1         + BETAEQ * 
     1           ((EIGHT/NINE)*(PVBBAR(1)**TWO) 
     1          -((TWO*EIGHT)/NINE)*((PVBBAR(2)**TWO)+(PVBBAR(3)**TWO)))
      CAB(2,1) = CAB(1,2)
      CAB(3,1) = CAB(1,3)
      CAB(3,2) = CAB(2,3)
! 
!     CALCULATING GAB 
     ...
     1                 + ALPHAEQ * 
     1                   ((EIGHT/THREE)*PVBBAR(J) 
     1                   + (FOUR/THREE)*PVBBAR(K))
     1                 + (DBETAEQDI1*DI1DEPS(J)+DBETAEQDI2*DI2DEPS(J)) *
     1                   (TWO*(PVBBAR(J)**TWO) - TWO*(PVBBAR(K)**TWO))
     1                 + BETAEQ * 
     1                   (((TWO*EIGHT)/THREE)*(PVBBAR(J)**TWO) 
     1                   + (EIGHT/THREE)*(PVBBAR(K)**TWO))
            ELSE
...
!     CALCULATE JACOBIAN OF OLD DEFORMATION GRADIENT
     ...
     1         -DFGRD0(1,3)*DFGRD0(3,1)*DFGRD0(2,2)
     1         -DFGRD0(2,3)*DFGRD0(3,2)*DFGRD0(1,1)
      END IF
! 
...
! 	CALCULATE TIME DERIVATIVE OF DEVIATORIC DAMAGE FUNCTION
	DDAMAGEDDT=AEXPD*BEXPD*((STATEV(NSTATV-3)-
     1           DAMVARINIT)**(AEXPD-ONE)) *
     1    EXP( - BEXPD*( ( STATEV(NSTATV-3) - DAMVARINIT )**AEXPD ) )*
     1    DDAMVARDDT*( ONE - W_PH * ( ONE - EXP(-PRESSURE/P_SAT) ) )
...
! ---------------------------------------------------------------------
!     ADDITIONAL STRESS COMPONENTS IN HYDRID FORMULATION
      DO N_B =1, N_BRANCHES
! Newton-Raphson for Je update (implicit)
! Initial guess: use explicit update as initial value
        Jv_n = Jv(N_B)
        CALL integrate_Jv(AJHAT, Jv_n, DTIME, ETAVOL(N_B),KVIS(N_B), Jv_new)
! Update Jv
        STATEV(7 + 7*(N_B - 1) ) = Jv_new
        Je(N_B)=AJHAT / Jv_new
! _________________________________________________________
!    Modification required when changing the viscous par of U(J)
!    See the documentation attached
! _________________________________________________________
        DUEDJHATNB= DUEDJHATNB+(KVIS(N_B)/TWO)*(Je(N_B)**2-ONE)/AJHAT
        D2UEDJHAT2NB = D2UEDJHAT2NB+KVIS(N_B)/TWO*(Je(N_B)**2+ONE)
     1                /AJHAT**2
        D3UEDJHAT3NB = D3UEDJHAT3NB-KVIS(N_B)/AJHAT**3
      END DO
      PHAT   = DUEDJHAT+DUEDJHATNB
      AKHAT2 = DET * (D2UEDJHAT2+D2UEDJHAT2NB)
      AKHAT3 = DET * (D3UEDJHAT3+D3UEDJHAT3NB) 
!
...
!         VISCOELASTIC LOOP
          DO N_B = 1, N_BRANCHES
            STRESSTOT(J,K)=STRESSTOT(J,K)+(ONE-DAMAGED)
     1                    *TAUNEQD(N_B,J,K)/DET
          END DO
        END DO
      END DO

!     VOIGT NOTATION FOR CAUCHY STRESS
      STRESS(1) = STRESSTOT(1,1) + (ONE-DAMAGEH)*PHAT
      STRESS(2) = STRESSTOT(2,2) + (ONE-DAMAGEH)*PHAT
      STRESS(3) = STRESSTOT(3,3) + (ONE-DAMAGEH)*PHAT
      STRESS(4) = STRESSTOT(1,2)
      STRESS(5) = STRESSTOT(1,3)
      STRESS(6) = STRESSTOT(2,3)
! 
!     REQUIRED STRESS COMPONENTS WHEN HYDRID FORMULATION  
      STRESS(NTENS+2) = AKHAT2
      STRESS(NTENS+3) = AKHAT3
!  
...
!         VISCOELASTIC LOOP
      ...
      END DO
! 
!     TOTAL ABAQUS TANGENT STIFNESS MATRIX in HYBRID FORMULATION 
      DDSDDE(1,1) = DDSDDE(1,1) + (ONE-DAMAGEH_OLD)*AKHAT2
      DDSDDE(2,2) = DDSDDE(2,2) + (ONE-DAMAGEH_OLD)*AKHAT2
      DDSDDE(3,3) = DDSDDE(3,3) + (ONE-DAMAGEH_OLD)*AKHAT2
      DDSDDE(1,2) = DDSDDE(1,2) + (ONE-DAMAGEH_OLD)*AKHAT2
      DDSDDE(1,3) = DDSDDE(1,3) + (ONE-DAMAGEH_OLD)*AKHAT2
      DDSDDE(2,3) = DDSDDE(2,3) + (ONE-DAMAGEH_OLD)*AKHAT2
      DDSDDE(2,1) = DDSDDE(2,1) + (ONE-DAMAGEH_OLD)*AKHAT2
      DDSDDE(3,1) = DDSDDE(3,1) + (ONE-DAMAGEH_OLD)*AKHAT2
      DDSDDE(3,2) = DDSDDE(3,2) + (ONE-DAMAGEH_OLD)*AKHAT2 
...
!   SUBROUTINES
!-----------------------------------------------------------
      SUBROUTINE integrate_Jv(J, Jv_old, dt, eta, K, Jv_new)
!-----------------------------------------------------------
      implicit none
      double precision, intent(in)  :: J, Jv_old, dt, eta, K
      double precision, intent(out) :: Jv_new
!  Local variables
      double precision :: R, dR, A, Jv, tol
      ...
      damping   = 0.5d0
      A  = dt * J * K / (2.0d0 * eta)
!      print *, 'A',A,dt,J,K,eta
      Jv = Jv_old  ! Initial guess

      do iter = 1, max_iter
! Residual
        R = Jv - Jv_old -A*(J/Jv - Jv/J)
! Derivaive of residual
        dR = 1.0d0 + A * (J / (Jv * Jv) + 1.0d0 / J)
! Newton-Raphson update
!        print *, R,Jv 
        Jv = Jv - damping* R / dR
!        if (abs(R) < tol) print *, 'good' 
        if (abs(R) < tol) exit
      end do
!
      if (iter >= max_iter) then
        print *, 'Warning: Newton-Raphson did not converge for Jv'
      end if
      Jv_new = Jv
      END SUBROUTINE integrate_Jv
...
!           DAMAGEVAR 4: heq = sqrt( (2/3)*(h1**2 + h2**2 + h3**2) )  
            ELSEIF (DAMCHOICE == 4) THEN
              DO J=1,3
                DAMVAR=DAMVAR+(TWO/THREE)*(((ONE/TWO)
     1                 *LOG(PVBBAR(J)))**TWO)
              END DO
              DAMVAR = SQRT(DAMVAR)
              DAMVARINIT = ZERO
